...
describe("failure and blocked classifications E2E", () => {
  ...
  it(
    "labels the issue ai-failed and writes a failure note when codex reports turn/failed",
    async () => {
      const ISSUE_IID = 11;
  ...
  it(
    "labels the issue ai-blocked without retrying when GitLab returns 403",
    async () => {
      const ISSUE_IID = 12;
      ws = await createE2EWorkspace({
  ...
  it(
    "auto-approves codex approval requests under approval_policy=never",
    async () => {
      const ISSUE_IID = 13;
      ws = await createE2EWorkspace({

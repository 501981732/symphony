...
- [ ] **步骤 1：写失败测试 → 步骤 2：跑测试确认失败 → 步骤 3：实现 → 步骤 4：跑 PASS → 步骤 5：提交**
...
- `running` / `pending`：什么也不动，写 `ci_status_observed`。
...
- [ ] **步骤 1：补 spec 链接 → 步骤 2：README 标 ✅ → 步骤 3：CHANGELOG 一条 → 步骤 4：release safety → 步骤 5：提交**

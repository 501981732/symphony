...
状态：V1 本地安装路径已闭合；公开 npm/internal registry 发布和升级策略仍留到后续 release 管理。

影响：

- ✅ 本地试运行可以通过 `pnpm release:pack` 生成 tarball，并用
  `npm install -g ./dist/release/issuepilot-0.1.0.tgz` 安装 `issuepilot` 命令。
- ✅ 安装态 CLI 支持 `issuepilot run --workflow ...` 和 `issuepilot dashboard`。
- 🚧 多项目/团队试用前，registry、版本 tag 和升级策略仍需明确。

收口方案：

1. V1 先使用 npm-compatible local tarball，不依赖 registry 凭据。
2. source checkout 继续作为贡献者开发和紧急回滚路径。
3. V2/后续 release 再决定 internal registry、升级命令和团队分发策略。
...
2. 明确 installable CLI local tarball path。
3. 保留 source checkout 作为 contributor / rollback path。

验收：

- 一次真实 GitLab smoke 可以按模板记录完整 evidence。
- README 不再把人工 smoke 暗示成自动 release gate。
- `pnpm release:check` 覆盖安装态 CLI smoke。

...
- `packages/workflow`：`.agents/workflow.md` 解析、校验、默认值、hot reload、prompt render。
...
| `WORKFLOW.md` | `.agents/workflow.md` | 有意替换；README 和 getting-started 统一使用 `.agents/workflow.md` |
...
#### Gap P0-2：事件契约还没有完全收窄到 shared contracts
...
#### Gap P0-3：Codex user-input-required 行为和 IssuePilot 主设计存在语义偏差
...
#### Gap P0-4：post-run MR 查找/upsert 边界表达不清
...
#### Gap P0-5：workflow reload 的动态效果不完整
...
#### Gap P0-6：workspace cleanup / retention 只有失败保留，没有完整策略
...
#### Gap P0-7：真实 smoke 仍偏人工 runbook，不是可重复 release gate
...
#### Gap P0-8：release / install / upgrade 路径未闭合
...
1. 不建议为了贴合根 `SPEC.md` 回退到 Linear / `WORKFLOW.md` / state-based tracker。那会违背 IssuePilot 已确认产品方向。
...
3. post-run reconciliation 对 existing MR 是显式 update，不靠 create side effect。
4. user-input-required 不会卡住，并按文档进入 blocked 或 failed。
5. workflow reload 的动态/需重启字段有明确说明。
6. P0 event contract 由 shared contracts 表达，dashboard 不直接依赖 runner raw payload。
7. workspace retention 策略在 P0 文档中说清楚。
8. 真实 GitLab smoke 有固定 evidence 模板。
9. 文档和测试通过：
